...

        {
            TagRegistry.registerDefault(
                    TextTag.enclosing(
                            ...
                            true,
                            (nodes, data, parser) -> {
                                return new ColorNode(nodes, TextColor.parseColor(data.get("value", 0, "white")).result().orElse(DEFAULT_COLOR));
                            }
                    )
            );
        }
                    ...
                    (nodes, data, parser) -> {
                        if (!data.isEmpty()) {
                            for (ClickEvent.Action action : ClickEvent.Action.values()) {
                                if (action.getSerializedName().equals(data.getNext("type"))) {
                                        ...
                                        if (type.equals("show_text") || type.equals("text")) {
                                            ...
                                            try {
                                                return new HoverNode<>(nodes,
                                                        ...
                                                        new HoverEvent.ItemStackInfo(ItemStack.of(TagParser.parseTag(value)))
                                                );
                                            } catch (Throwable e) {
                    ...
                    TextTag.enclosing(
                            ...
                            (nodes, data, parser) -> {
                                ...
                                while (true) {
                                    ...
                                    TextColor.parseColor(part).get().ifLeft(textColors::add);
                                }
                                return new GradientNode(nodes, switch (type) {
